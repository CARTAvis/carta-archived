...
(function (scope) {
    ...
    function formatMillis(t, prefix /* = "" */, width /* = 10 */) {
        if (typeof width !== "number") width = 6;
        prefix = "";
...
// convenience function for pureweb.getFramework().getState().setValue(key, val);
...
function setStateSize(num) {
    ...
    while (glob.usedUpVarCount < num) {
        ...
        stateSet(glob.varPrefix + "/" + varName, "x");
        glob.usedUpVarCount++;
    }
...
function updateImage() {
    Qt.makeNextImage( stamp ++);
//    var context = document.getElementById("cvs2").getContext("2d");
//    var imgData = Qt.img.toImageData();
//    var img = Qt.img;
//    var imgData = img.toImageData();
//    Qt.dbg( "imgData retrieved");
//    context.putImageData(Qt.img.toImageData());
//    Qt.img.assignToHTMLImageElement(document.getElementById("mimg"));
    var img = Qt.img;
    if( imgEl == null) {
...
function pongHandler(e) {
    ...
    if (!glob.continuePingPongFn(tDiff)) return;
...
var sendPing = (function () {
    ...
    return function () {
        ...
        stateSet(glob.varPrefix + "/ping", uniquePingId);
        uniquePingId++;
    }
})();
...
function onLoadCB() {
    ...
    pureweb.getFramework().getState().getStateManager().addValueChangedHandler(
            glob.varPrefix + "/pong", pongHandler);
