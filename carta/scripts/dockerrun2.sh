...
if [ $# -eq 0 ] ; then
    docker run --rm -p 8080:8080 -p 9999:9999 -ti --name carta2 \
    -e DISPLAY=$DISPLAYARG \
    -e SHELL="/bin/bash" \
    -v /tmp/.X11-unix:/tmp/.X11-unix -v $DIR/../../:/root/src/CARTAvis \
    grimmer0125/centoscarta:v8-improvebuildcasa /bin/bash
else
