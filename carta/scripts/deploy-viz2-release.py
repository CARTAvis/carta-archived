...
print "Hello from deploy-viz2-release.py"



# intended usage:
# 1) cd into build directory where the release was compiled using the buildRelease.py
#
...
env.user = "calsci"
env.hosts = ['viz2.cyberska.org']
pureWebRoot = "PureWeb/4.1.1"
remoteDeployDir = "CartaViewerRelease"
...
def checkLocalDirectory():
    ...
    ok = ok and os.path.isfile("cartaviewer")
    if not ok:
...
def deploy():
    checkLocalDirectory()
    # makeClean()
    puts("Deploying to %(host)s as %(user)s" % env)
