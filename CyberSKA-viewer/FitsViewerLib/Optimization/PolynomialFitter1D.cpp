...
VD PolynomialFitter1D::fit(int nTerms, const std::vector<PolynomialFitter1D::Pair> &data)
{
...
bool GuoFit::iterate()
{
    try {
        ...
        dbg(-1) << "gsl exception in iterate:" << e.what() << "\n";
        return false;
    } catch ( const std::runtime_error & e) {
        dbg(-1) << "runtime exception in iterate:" << e.what() << "\n";
        return false;
    } catch (...) {
        dbg(-1) << "exception in iterate!!!\n";
        return false;
    }

}

bool GuoFit::runCaruana()
{
//    dbg(1) << "Running caruana\n";
    ...
    for( size_t i = 0 ; i < m_data.size() ; i ++ ) {
        ...
        if( ! (y > EPS)) continue;  // skip negatives & NANs
...
//           << "sumxxlny = " << sumxxlny << "\n";
    ...
    double aa[] = { N,    sumx,   sumxx,
                    sumx, sumxx, sumxxx,
                    sumxx, sumxxx, sumxxxx };
    double bb[] = { sumlny, sumxlny, sumxxlny };
...
bool GuoFit::runNonIterativeGuo()
{
...
//           << "sumxxyylny = " << sumxxyylny << "\n";
    ...
    double aa[] = { sumyy, sumxyy, sumxxyy,
                    sumxyy, sumxxyy, sumxxxyy,
                    sumxxyy, sumxxxyy, sumxxxxyy };
    double bb[] = { sumyylny, sumxyylny, sumxxyylny };
