...
//    dbg(1) << "Trace astGCap(" << cap << "," << value << ")\n";
    if( cap == GRF__MJUST) return 1;
    if( cap == GRF__ESC) return 0;
    if( cap == GRF__SCALES) return 0;
    return 0;
}
...
//    dbg(1) << "Trace GMARK ==========\n";
    ...
    if( n > 0 && x && y ) {
        //       ccpgpt( n, (float *) x, (float *) y, type );
    }
    for( int i = 0 ; i < n ; i ++ ) {
        p().drawEllipse( x[i], y[i], type, type);
    }
    return 1;
}

static void drawText( const char *text, float x, float y, const char *just,
...
//    dbg(1) << "Trace: astGQch\n";
    ...
    if( chh) *chh = p().font().pointSizeF();
    if( chv) *chv = p().font().pointSizeF();
    return 1;
}
...
*-
*/
//    dbg(1) << "Trace astGAttr( " << attr << "," << value << "," << old_value << ","
//           << prim << ")\n";
...
//        out << "width(";
        if( old_value) * old_value = p().pen().widthF();
        if( value != AST__BAD) {
            QPen pen = p().pen();
            pen.setWidthF( value);
            p().setPen( pen);
...
//        out << "size(";
        if( old_value) * old_value = p().fontInfo().pointSizeF();
        if( value != AST__BAD) {
            QFont font = p().font();
