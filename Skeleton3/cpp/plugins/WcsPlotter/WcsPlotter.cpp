...
typedef Carta::Lib::Hooks::DrawWcsGrid DrawWcsGrid;

WcsPlotter::WcsPlotter( QObject * parent ) :
    QObject( parent )
{ }

bool
WcsPlotter::handleHook( BaseHook & hookData )
{
...
//        sgp.setOutputRect( QRectF( QPointF(0,0), params.outputSize));
...
//        sgp.setPlotOption( "Labelling=Interior");
    ...
    qWarning() << "Sorrry, WcsPlotter doesnt' know how to handle this hook" << hookData.hookId();
    return false;
}

std::vector < HookId >
WcsPlotter::getInitialHookList()
{
    return {
               Initialize::staticId,
