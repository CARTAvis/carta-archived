...
qx.Class.define("skel.widgets.MenuBar",
    {
        ...
        members: {
        	...
        	_initMenu: function(){
        	    ...
        	    layoutImageButton.addListener("execute", function () {
        	    	this.fireDataEvent( "layoutImage",""); 	
                }, this);
        	    layoutMenu.add( layoutImageButton ); 
        	    var layoutImageAnalysisControlsButton = new qx.ui.menu.Button( "Image + Analysis + Animator");
        	    layoutImageAnalysisControlsButton.addListener("execute", function () {
        	    	this.fireDataEvent( "layoutImageAnalysisAnimator", "");
                }, this);
        	    layoutMenu.add( layoutImageAnalysisControlsButton );
        	    var layoutCustomButton = new qx.ui.menu.Button( "Custom...");
        	    layoutCustomButton.addListener( "execute", function(){
        	    ...
        	    menuShownButton.addListener( "execute", function(){
        	    	this.setAlwaysVisible( this, menuShownButton.getValue());
        	    	this.fireDataEvent( "menuAlwaysVisible", menuShownButton.getValue());
        	    }, this );
        	    menuBarMenu.add( menuShownButton );
        	    this.m_menuPositionButton = new qx.ui.menu.Button("Side");
        	    this.m_menuPositionButton.addListener( "execute", function(){
        	    	this._reposition();
        	    }, this );
        	    ...
        	    statusShownButton.addListener( "execute", function(){
        	    	this.fireDataEvent( "statusAlwaysVisible", statusShownButton.getValue());
        	    }, this );
        	...
        	animateEnd : function( show ){
        		 if ( show ){
    				 if ( this.isTop() ){
    					 ...
    					 this.setLayoutProperties({bottom:heightPercent});
    				 }
    				 else {
    				 ...
    				 if ( this.isTop() ){
    					 ...
    					 this.setLayoutProperties({bottom: "100%"});
    				 }
    				 else {
    					 this.setWidth( 0 );
    					 this.setMinWidth( 0 );
        	...
        	animateSize : function( percent, show ){
        		 if ( show ){
        			 if ( this.isTop()){			
        				 var calc = Math.round((1-percent) *(100 - this.m_animationSize) + this.m_animationSize);
    					 var calcStr = calc + "%";
    					 this.setLayoutProperties( {bottom:calcStr});
        			 }
        			 else {	 
        	...
        	/**
        	 * Returns whether or not the menu bar is currently positioned at the
        	 * top of the display area.
        	 */
        	isTop : function(){
        		var topPosition = true;
        		//User has the option of changing it to top, which means it is not 
        		//currently top.
        	...
        	/**
        	 * Adds menu buttons to the application menu based on the window
        	 * that is selected.
        	 */
        	addWindowMenu : function( ev ){
        		if ( this.m_windowButtons != null ){
        			for ( var i = 0; i < this.m_windowButtons.length; i++ ){
        				this.m_menuPart.remove( this.m_windowButtons[i]);
        			}
        		}
        	 ...
        	 */
        	  reposition : function(){
          		if ( this.isTop() ){
          			...
          			this.setLayoutProperties( {left: "0%", right: "0%", top: "0%", bottom: "100%"});
          		}
          		else {
          			var menuWidth = 120;
          			...
          			this.setHeight( null );
          			this.setLayoutProperties( {left: "0%", bottom: "0%", top: "0%", right: "90%"} );
          		}           	
              },
        	...
        	_setAnimationSize : function(){
        		...
        		if ( this.isTop() ){
            		...
            		this.m_animationSize = 100 - Math.round( statusHeight / appHeight * 100 );
        	 ...
        	 */
             ...
             _showCustomLayoutPopup : function(){
            	 if ( this.m_layoutCustom == null ){
     	    		...
     	    		this.m_layoutCustom.addListener( "rowCount", function(ev){
     	    			var rowCount = ev.getData();
     	    			this.fireDataEvent( "layoutRowCount", rowCount);
     	    		}, this);
     	    		this.m_layoutCustom.addListener( "colCount", function(ev){
     	    	 ...
     	    	 this.m_layoutCustom.placeToPoint( {left:rightButton, top:topButton}, false );
