...
qx.Class.define("skel.widgets.DisplayWindow",
	...
	construct: function ( pluginId, row, col) {
		...
		this._init();
	     ...
	     this.m_sharedVarPlugin = this.m_connector.getSharedVar( pathDict.PLUGIN_LIST_STAMP );
	...
	members: {
	    ...
	    /**
         * Returns this window's information concerning establishing a link from the window
         * identified by the sourceWinId to this window.
         * @param pluginId {String} the name of the plug-in displayed by the source window.
		...
		_initContextMenu: function(){
			...
			this.m_linkButton.addListener( "execute", function(){
				...
				qx.event.message.Bus.dispatch( 
						new qx.event.message.Message("showLinks", linkData));
			}, this );
		...
		_initDataMenu : function(  ){
			...
			openButton.addListener( "execute", function(){
				qx.event.message.Bus.dispatch( 
						new qx.event.message.Message("showFileBrowser", this));
			}, this);
		...
		/**
		 * Initialize the submenu displaying alternative plug-ins to view
		 * in this window.
		 */
            ...
            var pluginCount = parseInt( val);
             ...
             for( var i = 0 ; i < pluginCount ; i ++ ) {
                 ...
                 nameButton.addListener( "execute", function(){
                	 ...
                	 qx.event.message.Bus.dispatch( 
        						new qx.event.message.Message("setView", data));
                 }, nameButton);
		 ...
		 */
		 ...
		 setSelected : function ( selected, multiple ){
			 if (selected ){
				 ...
				 if ( !multiple ){
					 qx.event.message.Bus.dispatch( 
								new qx.event.message.Message("windowSelected", this));
				 }
			 }
			 else {
