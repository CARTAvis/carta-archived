...
qx.Class.define("skel.widgets.StatusBar",
	...
	construct: function () {
		...
		this.m_sharedContainer.add( this. m_sharedUrl, {flex:1} );
	...
	members: {
		/**
		 * Add information to the status bar indicating the url of the shared session.
		 */
		...
		addWidgets : function(){
			if ( !this.hasLayoutChildren() ){
				...
				if ( url && url.length > 0 ){
					this._add( this.m_sharedContainer, {flex:1} );
				}
			}
		},
