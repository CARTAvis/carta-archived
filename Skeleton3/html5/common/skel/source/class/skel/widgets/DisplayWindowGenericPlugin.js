...
qx.Class.define("skel.widgets.DisplayWindowGenericPlugin",
    {
        ...
        members: {
            ...
            isLinkable : function( pluginId ){
            	var linkable = false;
            	if ( pluginId  == "casaLoader" && this.m_pluginId == "statistics"){
            		linkable = true;
            	}
            	else if ( pluginId == "animator" && this.m_pluginId != "plugins"){
            		linkable = true;
            	}
            	return linkable;
            },
            ...
            changeLink : function( sourceWinId, destWinId, addLink ){
            	...
            	if ( destWinId == this.m_identifier ){
            		...
            		if ( addLink && linkIndex < 0 ){
	            		...
	            		//Right now only generic support is statistics.  Need to generalize.
	            		if ( this.m_title.getValue() == "statistics" ){
	                  		var mouseXPath = skel.widgets.Path.getInstance().MOUSE_X + sourceWinId;
	                		var labelx = new skel.boundWidgets.Label( "MouseX:", "pix", mouseXPath );
	                        this.m_content.add( labelx);
	                        var mouseYPath = skel.widgets.Path.getInstance().MOUSE_Y + sourceWinId;
