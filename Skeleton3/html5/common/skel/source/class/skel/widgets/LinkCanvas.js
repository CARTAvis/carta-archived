...
qx.Class.define("skel.widgets.LinkCanvas",
 {
	...
	construct : function(){
         ...
         //In order to receive key events, activate must be called.
    ...
    members : {
       ...
       /**
        * Draws a connector between a source point and a destination point.
        * @param startX {Integer} the x-coordinate of the source point.
        * @param startY {Integer} the y-coordinate of the source point.
        * @param endX {Integer} the x-coordinate of the destination point.
        * @param endY {Integer} the y-coordinate of the destination point.
        * @param ctx {CanvasRenderingContext2D} the rendering context.
        */
       _drawConnector : function( startX, startY, endX, endY, biLink, ctx ){
    	   this._drawLine( startX, startY, endX, endY, ctx );
    	   
    	   var arrow1End = this._getArrowEnd( startX, startY, endX, endY, true );
    	   this._drawLine( endX, endY, arrow1End.x, arrow1End.y, ctx );
    	   
    	   var arrow2End = this._getArrowEnd( startX, startY, endX, endY, false );
    	   this._drawLine( endX, endY, arrow2End.x, arrow2End.y, ctx );
    	   if ( biLink ){
    		   var arrow3End = this._getArrowEnd( endX, endY, startX, startY, true );
        	   this._drawLine( startX, startY, arrow3End.x, arrow3End.y, ctx );
        	   
        	   var arrow4End = this._getArrowEnd( endX, endY, startX, startY, false );
        	   this._drawLine( startX, startY, arrow4End.x, arrow4End.y, ctx );
    	   }
       },
       ...
       /**
        * Draws a mark indicating a source or destination point for a connector.
        * @param linkInfo {skel.widgets.LinkInfo} representation for an end point of a link connector.
        * @param ctx {CanvasRenderingContext2D} the rendering context.
        */
       ...
       /**
        * Draws a link connector from the source link to the current mouse position
        * during an interactive user draw. 
        * @param ctx {CanvasRenderingContext2D} the rendering context.
        */
       ...
       _drawNewLink : function( ctx ){
    	   if ( this.m_linkStart != null && this.m_linkEnd != null ){
    		   ...
    		   this._drawLine( this.m_linkStart.x, this.m_linkStart.y, this.m_linkEnd.x, this.m_linkEnd.y, ctx );
    	   }
       },
       
       /**
        * Returns the link source or destination that matches the passed in point or
        * null if there is no link end point 'close' to the point.
        * @param pt {Array} consisting of a screen pixel coordinate.
        */
       ...
       /**
        * Event callback when the users types a key on the keyboard.
       ...
       /**
        * Returns the destination link point of the line connector 'close to' the point or null
        * if there is no line connector near the point.
       ...
       _matchesLine : function( pt ){
    	   ...
    	   for ( var i = 0; i < this.m_destLinks.length; i++ ){
    		   ...
    		   if ( distance < this.m_ERROR_MARGIN ){
    			   //Make sure the point is in the segment range.
    			   var minX = Math.min( this.m_sourceLink.locationX, this.m_destLinks[i].locationX );
    			   var maxX = Math.min( this.m_sourceLink.locationX, this.m_destLinks[i].locationX );
    			   if ( minX == maxX || (minX <= pt.x && pt.x<= maxX )){
    				   var minY = Math.min( this.m_sourceLink.locationY, this.m_destLinks[i].locationY );
        			   var maxY = Math.min( this.m_sourceLink.locationY, this.m_destLinks[i].locationY );
       ...
       /**
        * Returns true if the passed in link end point is 'close to' the screen coordinate point; false
        * otherwise.
        * @param pt {Array} a screen coordinate.
       ...
       _mouseDownCB : function( ev ){
           ...
           if( ev.getButton() === "left" ) {
        	   ...
        	   var sourceMatch = this._matchesPoint( pt, this.m_sourceLink );
        	   if ( sourceMatch ){
        		   this.capture();
        		   this.m_linkStart = {
           ...
           //Show the context menu if the point is "close to" a line connector.
        	   ...
        	   if ( lineMatch != null ){
        		   if ( this.m_contextMenu == null ){
        	           ...
        	           var removeLinkButton = new qx.ui.menu.Button( "Remove Link");
        	           removeLinkButton.addListener( "execute", function(){
       ...
       _mouseUpCB : function( ev ){
           ...
           if( ev.getButton() === "left") {
        	   ...
        	   var matchingLink = this._getDestinationMatch( pt );
